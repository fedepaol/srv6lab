...
ip -6 addr add fc00:0:3::1/128 dev lo
ip -6 addr add fd00:3::1/128 dev lo
ip addr add 10.100.0.3/24 dev eth1
ip -6 addr add fc00:100::3/64 dev eth1
ip link add red type vrf table 1100
ip link add lored type dummy
ip link set lored up
ip link set lored master red
ip addr add 10.10.3.1/32 dev lored
ip -6 addr add fc00:10:3::1/128 dev lored
ip link set red up

# L2 EVPN VXLAN overlay
ip link add br10 type bridge
ip link set br10 master red
ip link set br10 addr aa:bb:cc:00:00:03
ip addr add 192.168.10.3/24 dev br10

ip link add vni110 type vxlan local 10.0.0.3 dstport 4789 id 110 nolearning
